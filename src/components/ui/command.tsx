  ...
  return (
    <Dialog {...props}>
      <DialogContent className="overflow-hidden p-0 sm:max-w-[640px]">
        {/* H27 : Title/Description DOIVENT être descendants de DialogContent
...
function CommandItem({
  ...
  return (
    <CommandPrimitive.Item
      ...
      className={cn(
        "relative flex cursor-pointer items-center gap-2 rounded-md px-2 py-1.5 text-sm outline-hidden select-none data-[selected=true]:bg-accent data-[selected=true]:text-accent-foreground data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}
    />
...
function CommandShortcut({
  ...
  return (
    <span
      ...
      className={cn(
        "ml-auto text-[10px] tracking-wider text-muted-foreground font-mono",
        className
      )}
      {...props}
    />
